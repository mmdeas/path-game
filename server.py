...
class Server(object):
	...
	def start(self):
		...
		if len(players) <= 4:
			...
			startPoints = [(0, size[1]-1), (size[0]-1, 0), (size[0]-1, size[1]-1), (0, 0)]
			endPoints = [startPoints[1], startPoints[0], startPoints[3], startPoints[2]]
		else:
			# Otherwise have random start points and finish in the opposite quarter
			startPoints = []
			endPoints = []
			for p in players:
				startPoints.append((randint(0, size[0]-1), randint(0, size[1]-1)))
	...
	def doMoves(self):
		...
		while not self.finished:
			...
			for i in xrange(len(self.players)):
				try:
					turn.append(self.moves.get(True, self.gameType.timeout/1000.*1.5)) # (client, node)
				except Empty, e:
...
class GameClient(ChatClient):
	...
	def gameStarted(self, start, end, players, costdelta):
		...
		self.visited = {start:()}
		self.childMaker = util.ChildMaker(self.server.imageSize, self.server.gameType.diagonals)
		self.start = start
		self.end = end
	...
	def perspective_expandNode(self, node, parent):
		...
		if node == (): # no-op
			...
			return
		log.msg({"parent": parent, "visited": self.visited, "node": node, "children":self.childMaker.getChildren(parent)})
		if parent not in self.visited or node not in self.childMaker.getChildren(parent):
...
class GameType(object):
	...
	"""
	def __init__(self, image, game='race', timeout=1000, automated=False, diagonals=True):
		"""
		image: filename of image to use for the game
		game: one of 'race' or 'battle'
		timeout: the time in ms a client waits for user input before sending a no-op.
		automated: boolean giving whether scripting is allowed in the client.
		"""
		log.msg(["GameType.__init__", self, image, game, timeout, automated, diagonals])
		self.game = game
...
if __name__ == '__main__':
	...
	from twisted.internet import reactor, stdio, tksupport
	...
	if len(argv) == 1 or argv[1] == "--help":
		...
		server = Server(argv[1], argv[2], **dict([tuple(a.split("=")) for a in argv[3:]]))
